            ...
            for row in rows:
                if {team_short: pass_2} == {
                        ...
                        st.write(
                            {
                                ...
                                t("App URL"): row.App,
                                t("Video URL"): row.Video,
                                t("Abstract"): row.Abstract,
                            }
                        )

                    # Count existing members from Google Sheet and add the count to session state
                    member_list = [
                        member.strip("[]").replace("'", "").replace("'", "").strip()
                        for member in row.Participants.split(",")
                    ]
                    ...
                    if row.GitHub and row.App:
                        ...
                        if st.checkbox(
                            t("Overwrite"), disabled=st.session_state.disabled
                        ):
