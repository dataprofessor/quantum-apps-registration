...
def submit_page(rows):
    ...
    if st.session_state["disabled"]:
        st.info(t("Please reload this page to enable submission"))
        ...
        if len(team_name) > 0:
            ...
            for row in rows:
                if {team_short: pass_2} == {
                    ...
                    st.write(
                        f"{t('Welcome back')} **`{team_name}`**. {t('You can now edit your project and submit it to the competition')}."
                    )
                    if st.checkbox(t("Show registration details"), disabled=st.session_state.disabled):
                        st.write(
                            {
                                t("Team name"): row.Team,
                                t("Password"): row.Password,
                                t("Team Members"): row.Participants,
                                t("Mentor"): row.Mentor,
                                t("Category"): row.Category,
                                t("GitHub Repo"): row.GitHub,
                                t("App URL"): row.App,
                            }
                        )
                    ...
                    if row.GitHub and row.App:
                        st.write(
                            t("Your project is already submitted. Do you want to overwrite it?")
                        )
                        if st.checkbox(t("Overwrite"), disabled=st.session_state.disabled):
            ...
            if idx == len(rows.fetchall()):
                st.error(t("Incorrect password or team name"))
                st.stop()
