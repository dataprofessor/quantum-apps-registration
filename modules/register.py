...
def register_page(rows):
    ...
    if st.session_state["disabled"]:
        st.info(t("Please reload this page to enable registration"))
...
If the team name you have chosen is taken already, please choose a different name to make sure there
    ...
    if len(team_name) > 0:
        ...
        if " " in team_name:
            st.warning(t("Please remove spaces in your team name"))
            st.stop()
        ...
        for row in rows:
            ...
            if row_short == team_short:  # makes case insensitive (A-team = a-team)
                st.warning(t("That name is taken, please choose a different team name"))
                st.stop()
    ...
    if st.session_state["team_chosen"]:
        ...
        if pass_1 and pass_2:
            ...
            if pass_1 != pass_2:
                ...
                update_password()
                st.write(f"{t('Your team password is')} **`{pass_1}`**")
                st.write(
                    f"❗ {t('Be sure to remember your team name and password, they will be used to edit and submit your project')}"
                )
                st.write("---")
    ...
    if st.session_state.pwd:
        ...
        with st.expander(t("If your team has a Mentor enter details here:")):
        ...
        team_count = st.number_input(
            t("Number of people on the team:"),
            1,
            4,
        ...
        st.write(t("Review and confirm your entry:"))
        st.write(f"**{t('Team name')}: `{st.session_state.team_name}`**")
        st.write(f"**{t('Team Member(s)')}: `{st.session_state.members}`**")
        st.write(f"**{t('Category')}: `{st.session_state.category}`**")
        st.write(f"**{t('Password')}: `{st.session_state.password}`**")
        ...
        if submit:
            ...
            st.info(t("Team information submitted"))
            st.balloons()
            registration = pd.DataFrame(
                {
                    t("Team name"): [st.session_state.team_name],
                    t("Password"): [st.session_state.password],
                    t("Members"): [st.session_state.members],
                    t("Mentor"): [st.session_state.mentor_name],
                    t("Category"): [st.session_state.category],
                }
            )
            download = st.download_button(
                label=t("Download registration details"),
